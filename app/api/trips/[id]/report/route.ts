...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const calculateKMTransports = trip.trip_transports.map(t => t.transport.calculateKM);  

  let calculateKMTransport = false

  if (calculateKMTransports.some(Boolean)) {
    calculateKMTransport = true
  }

  const valorKm = (trip.endKM - trip.startKM) * trip.parameters_km.value
    ...
    <div class="container-finish">
        ...
        <table class="table table-bordered">
          ...
          <tbody>
            <tr>
              ...
              <td>R$ ${(trip.advance_value - valorKm - totalExpenses).toFixed(2).replace('.', ',')}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="table-responsive">
        <table class="table table-bordered">
          <thead>
            <tr>
              <th colspan="2" class="text-center">Meio(s) de Transporte(s)</th>
            </tr>
            <tr>
              <th>Nome</th>
              <th>Calcula o KM?</th>
        ...
        <table class="table table-bordered">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td class="text-right">R$ ${totalExpenses.toFixed(2).replace('.', ',')}</td>
            </tr>
            <tr>
              <td>Total</td>
              <td class="text-right"><strong>R$ ${(trip.advance_value - valorKm - totalExpenses).toFixed(2).replace('.', ',')}</strong></td>
            </tr>
          </tbody>
        </table>
      </div>
